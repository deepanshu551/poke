...
export default function Details() {
  ...
  useEffect(() => {
    if (showModal && descModalRef.current && !modalFocused) {
      descModalRef.current.focus();
      setModalFocused(true); // Prevent repeated focusing
    } else if (!showModal) {
      setModalFocused(false); // Reset focus tracking when modal closes
    }
  }, [showModal]); // Dependency is now only showModal
    ...
    <section className="poke-details">
      <div className="poke-details-desc">
        ...
        <div className={`poke-details-description`} ref={descriptionRef}>
          ...
          <button className="poke-show-more" onClick={openDesc}>
            <span >read more</span>
          </button>
        </div>
      </div>
      ...
      <div className={`desc-modal ${showModal ? "show-modal" : ""}`} ref={descModalRef} tabIndex="-1" >
        {totalDesc}
        <FontAwesomeIcon
          className="close-icon"
          onClick={openDesc}
          onKeyDown={openDescTab}
          icon={faXmark}
          tabIndex="0"
        />
      </div>
